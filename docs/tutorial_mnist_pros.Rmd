...
```{r}
cross_entropy <- tf$reduce_mean(-tf$reduce_sum(y_ * tf$log(y), reduction_indices=idx(2)))
```
...
First we'll figure out where we predicted the correct label. `tf$argmax` is an extremely useful function which gives you the index of the highest entry in a tensor along some axis. For example, `tf$argmax(y, idx(2))` is the label our model thinks is most likely for each input, while `tf$argmax(y_, idx(2))` is the true label. We can use `tf$equal` to check if our prediction matches the truth.

```{r}
correct_prediction <- tf$equal(tf$argmax(y, idx(2)), tf$argmax(y_, idx(2)))
```

Note that since arrays in the TensorFlow API are 0-based we use the `idx` function to specify a more R friendly 1-based index that is automatially converted to 0-based.
...
```{r}
W_conv2 <- weight_variable(shape = shape(5, 5, 32, 64))
b_conv2 <- bias_variable(shape = shape(64))
