...
Here we instead use the convenient `InteractiveSession` class, which makes TensorFlow more flexible about how you structure your code.  It allows you to interleave operations which build a [computation graph](https://www.tensorflow.org/get_started/basic_usage.html#the-computation-graph) with ones that run the graph.  This is particularly convenient when working in
interactively in the R console.  If you are not using an `InteractiveSession`, then you should build the entire computation graph before starting a session and
[launching the graph](https://www.tensorflow.org/versions/r0.10/get_started/basic_usage.html#launching-the-graph-in-a-session).
...
The role of the R code is therefore to build this external computation graph, and to dictate which parts of the computation graph should be run. See the [Computation Graph](https://www.tensorflow.org/get_started/basic_usage.html#the-computation-graph) section of [Basic Usage](https://www.tensorflow.org/versions/r0.10/get_started/basic_usage.html) for more detail.
...
y_ <- tf$placeholder(tf$float32, shape=list(NULL, 10L))
```
...
```{r}
W <- tf$Variable(tf$zeros(shape = list(784L, 10L)))
b <- tf$Variable(tf$zeros(shape = list(10L)))
```
...
Note also that indices (like the one used for `reduction_indices`) are 0-based within the TensorFlow API rather than 1-based as is typical with R vectors. This is because TensorFlow is implemented in Python which uses 0-based arrays.
...
```{r}
conv2d <- function(x, W) {
  tf$nn$conv2d(x, W, strides=list(1L, 1L, 1L, 1L), padding='SAME')
}

max_pool_2x2 <- function(x) {
  tf$nn$max_pool(
    x, 
    ksize=list(1L, 2L, 2L, 1L),
    strides=list(1L, 2L, 2L, 1L), 
    padding='SAME')
}
```
...
We can now implement our first layer. It will consist of convolution, followed by max pooling. The convolutional will compute 32 features for each 5x5 patch. Its weight tensor will have a shape of `(5L, 5L, 1L, 32L)`. The first two dimensions are the patch size, the next is the number of input channels, and the last is the number of output channels. We will also have a bias vector with a component for each output channel.

```{r}
W_conv1 <- weight_variable(shape = list(5L, 5L, 1L, 32L))
b_conv1 <- bias_variable(shape = list(32L))
```

To apply the layer, we first reshape `x` to a 4d tensor, with the second and third dimensions corresponding to image width and height, and the final dimension corresponding to the number of color channels.

```{r}
x_image <- tf$reshape(x, list(-1L, 28L, 28L, 1L))
```
...
```{r}
W_conv2 <- weight_variable(shape = list(5L, 5L, 32L, 64L))
b_conv2 <- bias_variable(shape = list(64))
