...
[![Travis-CI Build Status](https://travis-ci.org/rstudio/reticulate.svg?branch=main)](https://travis-ci.com/github/rstudio/reticulate) [![Appveyor Build Status](https://ci.appveyor.com/api/projects/status/github/rstudio/reticulate?svg=true)](https://ci.appveyor.com/project/rstudio/reticulate)
...
See the article on [Python Version Configuration](https://rstudio.github.io/reticulate/articles/versions.html) for additional details.
...
1) [Python in R Markdown] &mdash; A new Python language engine for R Markdown that supports bi-directional communication between R and Python (R chunks can access Python objects and vice-versa).

2) [Importing Python modules] &mdash; The `import()` function enables you to import any Python module and call its functions directly from R.

3) [Sourcing Python scripts] &mdash; The `source_python()` function enables you to source a Python script the same way you would `source()` an R script (Python functions and objects defined within the script become directly available to the R session).

4) [Python REPL] &mdash; The `repl_python()` function creates an interactive Python console within R. Objects you create within Python are available to your R session (and vice-versa).
...
![](images/rmarkdown_engine_zoomed.png){.screenshot}
...
![](images/reticulate_completion.png){.screenshot}
