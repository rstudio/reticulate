...
This means that when a Python API expects an integer, you need to be sure to use the `L` suffix within R. For example, if the `foo` function requires an integer as it's `index` argument you would do this:
...
If you are trying to pass an R function to your Python code, `r_to_py()` will be applied to the R function first to wrap the R function in a Python function with signature `function(...)` where there's neither keyword argument nor default value for each argument.

For example, below we apply `r_to_py()` to an R function and then we use [inspect Python module](https://docs.python.org/3/library/inspect.html) to get the function's argument spec of the Python function that wraps the R function. You can see that the signature of the wrapped function looks different than the original R function's signature.
...
This will cause issues sometimes since some Python libraries have very strict checking on the function signature that the user passes. In the above example, if the Python library you are using requires the function that you are passing to have arguments `a` and `b`, then this converted function will not be acceptable since it doesn't have any of those keyword arguments. 

In order to convert the R function correctly, you can use `py_func()` to wrap the R function so that the wrapped function has exactly the same signature as that of the original R function, e.g. one argument `a` without default value and another argument `b` with default value 1.5. 
