...
- Some APIs call for shapes, which are lists of integers that can include `NULL` for dimensions that can have any number of elements. For this use the shape function (e.g. `shape(NULL, 10L)`). The shape function handles `NULL`, ensures that single-element vectors are still passed as lists, and automatically converts it's arguments to integer.
