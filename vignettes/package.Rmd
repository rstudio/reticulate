...
> **NOTE**: The dependency installation features described below are currently only available in the development version of reticulate. You can install the development version with `devtools::install_github("rstudio/reticulate")`.
...
Here is the approach we recommend to installing Python dependencies required by R packages:

1) Ask your users to install the Anaconda Python distribution from this link: <https://docs.anaconda.com/anaconda/install/>

2) Provide an R function that installs the required Python package(s) into the "r-reticulate" Conda environment. For example:

    ```{r, eval = FALSE}
    install_scipy <- function(conda = "auto") {
      reticulate::conda_install("r-reticulate", "scipy", conda = conda)
    }
    ```

3) Then, provide a hint to reticulate that Python packages you are importing are likely to be found in the "r-reticulate" environment. Extending the `.onLoad()` example from above:
