...
The **tensorflow** package provides access to the entire TensorFlow API from within R.  This article describes the basic mechanics of using the R API. For a detailed API reference, see the main [TensorFlow API](https://www.tensorflow.org/versions/r0.10/api_docs/python/index.html) documentation. This documentation uses the Python API, however the R API is based on the Python API so documentation, tutorials, and examples that use Python are easily applied to R. 
