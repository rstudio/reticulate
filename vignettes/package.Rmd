...
Calling `py_require()` is always recommended because it has no effect if the
user is using a manually managed Python environment, making it generally safe to
use.

### Typical Usage

`py_require()` is typically called from `.onLoad()`, like this:
...
It can also be called from other package functions. For example, the `keras3` R
package defines a `use_backend()` function that calls `py_require()` with
different values based on the user-requested backend:

``` r
.onLoad <- function(...) {
  py_require(c("keras", "tensorflow")) # default tensorflow backend
}
...
prefer `>=` and `!=` over `<=`, as the latter can quickly become outdated.
...
For testing R packages with GitHub Actions, dependencies declared via `py_require()` will
resolve automatically with no additional steps. If there are extra Python test
dependencies, declare them using `py_require()` in `tests/testthat/helper.R`.
The standard R-CMD-check workflow should work:
