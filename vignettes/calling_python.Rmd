...
If you are trying to pass an R function to your Python code, `r_to_py()` will be applied to the R function first to wrap the R function in a Python function with signature `function(...)` where there's no keyword argument with no default for each argument. 
...
To correct this, you can use `py_func()` to wrap the R function so that the wrapped function has exactly the same signature as that of the original R function, e.g. one argument `a` without default value and another argument `b` with default value 1.5. 
