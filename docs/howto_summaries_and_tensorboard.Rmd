...
There are other resources available as well! The [TensorBoard README](https://www.tensorflow.org/code/tensorflow/tensorboard/)
...
attaching [`tf$summary$scalar`](https://www.tensorflow.org/api_docs/python/summary/generation_of_summaries_#scalar) ops
to the nodes that output the learning rate and loss respectively. Then, give
each `tf$summary$scalar` a meaningful `tag`, like `'learning rate'` or `'loss
function'`.
...
[`tf$summary$histogram`](https://www.tensorflow.org/api_docs/python/summary/generation_of_summaries_#histogram) ops to
...
[`tf$summary$merge_all`](https://www.tensorflow.org/api_docs/python/summary/generation_of_summaries_#merge_all)
...
[`tf$summary$FileWriter`](https://www.tensorflow.org/api_docs/python/summary/generation_of_summaries_#FileWriter).

The `tf$summary$FileWriter` takes a logdir in its constructor - this logdir is quite
important, it's the directory where all of the events will be written out.
Also, the `tf$summary$FileWriter` can optionally take a `Graph` in its constructor.
...
[Tensor shape information](howto_graph_viz.html#tensor-shape-information).

Now that you've modified your graph and have a `tf$summary$FileWriter`, you're ready to
...
The code example below is a modification of the [simple MNIST tutorial](tutorial_mnist_beginners.html), in which we have
...
The code below is an excerpt; full source is [here](https://github.com/rstudio/tensorflow/blob/master/inst/examples/mnist/mnist_with_summaries.R).
...
where `logdir` points to the directory where the `tf$summary$FileWriter` serialized its
...
see [TensorBoard: Graph Visualization](howto_graph_viz.html).
