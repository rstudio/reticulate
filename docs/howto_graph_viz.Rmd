...
![](images/graph_vis_animation.gif){.tutorial-illustration .tutorial-illustration-large}
*Visualization of a TensorFlow graph.*

To see your own graph, run TensorBoard pointing it to the log directory of the job, click on the graph tab on the top pane and select the appropriate run using the menu at the upper left corner. For in depth information on how to run TensorBoard and make sure you are logging all the necessary information, see [TensorBoard: Visualizing Learning](howto_summaries_and_tensorboard.html).

You can interact with an instance of TensorBoard looking at data from an
[MNIST](tutorial_mnist_beginners.html) training session, including the
...
[`tf$name_scope`](https://www.tensorflow.org/api_docs/python/framework.html#name_scope):
...
| Symbol | Meaning
| :-----: | -------------------------------------------------------------
| ![Name scope](images/namespace_node.png) | *High-level* node representing a name scope. Double-click to expand a high-level node.
| ![Sequence of unconnected nodes](images/horizontal_stack.png) | Sequence of numbered nodes that are not connected to each other.
| ![Sequence of connected nodes](images/vertical_stack.png) | Sequence of numbered nodes that are connected to each other.
| ![Operation node](images/op_node.png) | An individual operation node.
| ![Constant node](images/constant.png) | A constant.
| ![Summary node](images/summary.png) | A summary node.
| ![Data flow edge](images/dataflow_edge.png) | Edge showing the data flow between operations.
| ![Control dependency edge](images/control_edge.png) | Edge showing the control dependency between operations.
| ![Reference edge](images/reference_edge.png) | A reference edge showing that the outgoing operation node can mutate the incoming tensor.
...
(as in `sess.graph`) to the `tf$summary$FileWriter` when serializing the graph.
...
[simple MNIST tutorial](tutorial_mnist_beginners.html),
in which we have recorded summaries and runtime statistics. See the [Summaries Tutorial](howto_summaries_and_tensorboard.html#serializing-the-data)
for details on how to record summaries.
Full source is [here](https://github.com/rstudio/tensorflow/blob/master/inst/examples/mnist/mnist_with_summaries.R).
