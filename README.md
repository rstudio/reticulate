...
**OS X**: You should NOT install TensorFlow with Anaconda on OS X as there [are issues with](https://github.com/ContinuumIO/anaconda-issues/issues/498) the way Anaconda builds the python shared library on OS X that prevent dynamic linking from R.

If you install TensorFlow within a Virtualenv or Conda environment you'll need to be sure to use that same environment when installing the tensorflow R package (see below for details).
