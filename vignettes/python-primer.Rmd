...
R is the common language of statistics and data analysis, but it is not
the only language on the scene. You may find yourself occasionally
wanting to read and understand some Python, or even port some Python to
R. This guide is designed to enable you to do these tasks as quickly as
possible. As you'll see, R and Python are similar enough that it's
possible to do these tasks, without necessarily learning all of Python.

### Whitespace

Whitespace matters in Python. In R you groups expressions into a code
block with `{}`, in Python you do that by making the expressions share
...
`list()` is feature packed, and there is actually no single direct
equivalent in Python that supports all the same features. Instead there
are (at least) 4 different Python container types you need to be aware
of: lists, dictionaries, tuples, and sets.
...
usage of `+` and `*` with lists. These are concatenation operators, akin
to R's `c()`.
...
the slice syntax is *not* inclusive of the end of the slice range. You
can optionally also specify a stride.
...
used to group expressions to define order of operations in expressions
like `(x + 3) * 4`, a special syntax is required to define tuples of
length 1: a trailing comma. Tuples are most commonly encountered in
...
assign multiple symbols in one expression.
...
Tuple unpacking can occur in multiple contexts, such as iteration:
...
Dictionaries are most similar to R's named lists. They are a container
...
Unlike R's named lists, you cannot index into a dictionary with an
integer to get an item at a specific index position. Dictionaries are
*unordered* containers. (However---beginning with Python 3.7,
dictionaries do preserve the item insertion order).
...
`OrderedDict`, but that's relatively uncommon in Python code so we don't
cover it further.

#### Sets

Sets are a container that can be used to efficiently track unique items
or deduplicate lists. They are constructed using `{val1, val2}` (unlike
a dictionary, without `:`). Think of them as dictionary where you only
...
methods in Python. (In contrast to R's S3 approach, where methods are
found by dispatching from a generic). Fortunately, the basics of
`class`'s are accessible.
...
hint that it's common to be managing many instances of a class.
...
-   the `class` statement takes a code block that is defined by a common
    ...
    The body of the class is evaluated once, when the class constructor
...
class MyClass:
  def a_method(self):
    print("self.a_method() was called with", self)

inst = MyClass()
inst.a_method()

MyClass.a_method()     # error, missing required argument `self`
MyClass.a_method(inst) # identical to inst.a_method()
```
...
-   `__repr__`, `__str__`, functions invoked for formatting and pretty
    printing (equivalent to `format()` and `print()` S3 generics in R.

-   `__enter__` and `__exit__`: functions invoked by `with`.

-   Many builtin Python functions are just sugar for invoking the
    dunder. For example: calling `repr(x)` is identical to
    `x.__repr__()`. Other builtins that are just sugar for invoking the
    dunder are `next()`, `iter()`, `str()`, `list()`, `dict()`,
    `bool()`, `dir()` and more!
...
then return a function. Any function can be invoked as a decorator with
the `@` syntax, which is just sugar for this simple action:
...
iterator. Most features dealing with files are iterators, and so on. Any
time you find an iterator inconvenient, you can materialize all the
elements into a list using the Python built-in `list()`, or
`reticulate::iterate()` in R.
...
```{r}
library(reticulate)
py$a_strict_Python_function(3) |> try()    # error
...
R is a language designed for numerical computing first. Vector data
types are baked deep into the R language, to the point that the language
doesn't even distinguish scalars from vectors. By comparison, numerical
computing features in Python are generally provided by third party
packages (*modules*, in Python parlance). The closest equivalent to an R
numeric vector is a numpy array, or sometimes, a Python list of numerics
(some Pythonistas might argue for `array.array()` here, but that's so
rarely encountered in actual Python code we don't mention it further).
Teaching the NumPy interface is beyond the scope of this tutorial, but
...
-   Many numpy operations modify the array in place! This is surprising
    ...
    and conduct small experiments at the `reticulate::python_repl()`.
...
Python module named 'numpy', load it (commonly meaning: evaluate it's
...
and unexported symbols. In Python, all module symbols are equal, though
...
experiments at the `reticulate::python_repl()`.
